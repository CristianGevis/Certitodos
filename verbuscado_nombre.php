...
$resul_prov = mysqli_query($conex, "SELECT * FROM participantes WHERE nombre LIKE '%" . $busca . "%'");
...
$resul_prov = mysqli_query($conex, "SELECT * FROM participantes WHERE nombre LIKE '%" . $busca . "%' ORDER BY id LIMIT " . (($paginacion->get_page() - 1) * $resultados) . ", " . $resultados);
    ...
    <div id="stylized">
      <table border="0">
        <tr>
          ...
          <td width="250">
            <div style="width:220px; float:right; font-size:12px; background:#ffffff; padding:2px; width:100px; margin-bottom:15px"><button type="button" id="boton" onclick="window.location.href='admin.php'"> Ver Todos</button></div>
          </td>
        </tr>
      </table>
    </div>
